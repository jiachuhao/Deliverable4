...
public class MainActivity extends AppCompatActivity implements View.OnClickListener, AdapterView.OnItemSelectedListener {
    ...
    private DatabaseReference databaseReferenceRate;
    ...
    private void saveUserInfo(){
        ...
        try {
            newpasswrod=Sha1.hash(password);
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }

        Account account=new Account(user.getUid(),email,newpasswrod,type);
        databaseReference.child(user.getUid()).setValue(account);

        if(type.equals("Service Provider")){

        databaseReferenceRate=FirebaseDatabase.getInstance().getReference("Provider_Rate").child(user.getUid());
        Service_Provider_Rate service_provider_rate=new Service_Provider_Rate(0,0,0,user.getUid(),user.getEmail());
        databaseReferenceRate.setValue(service_provider_rate);
